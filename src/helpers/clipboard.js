    function clipboardInit(graph)
		{
            ...
            // Deletes selected element on delete evt.keyCode == '46' || 
            mxEvent.addListener(document, 'keyup', function(evt){
            if (evt.keyCode == '48' && (evt.ctrlKey || evt.metaKey))
                {
            ...
            mxEvent.addListener(textInput, 'copy', mxUtils.bind(this, function(evt)
            {
                ...
                {   
                    // Negar copias innecesarias
                    var selection = graph.getSelectionCells();
                    var init = selection.length;                    
                    //console.log("Total inicial...", selection.length);
                    //console.log("Total inicial...", selection);                    
                    for (var h = 0; h < init; h++) {
                        for (var i = 0; i < selection.length; i++) {
                            //console.log("Veces...", i);
                            if(selection[i].value.type === 'Support Actor'){
                                //console.log("Se seleccionó Support Actor");
                                selection.splice(i, 1);                                
                            };                        
                        }
                        for (var i = 0; i < selection.length; i++) {
                            if(selection[i].value.type === 'Internal Communicative Event'){
                                //console.log("Se seleccionó Internal Communicative Event");
                                selection.splice(i, 1);                            
                            };                        
                        }
                    }                    
                    //console.log("Total...", selection.length);
                    //console.log("Total...", selection);
                    copyCells(graph, mxUtils.sortCells(graph.model.getTopmostCells(selection)));
                    dx = 0;
                    dy = 0;
                }                
            }));
